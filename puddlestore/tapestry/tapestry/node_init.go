...
func (local *Node) GetNodeAddr() string {
	node := local.node
	addr := node.Getaddr()
	return addr
}
...
func start(id ID, port int, connectTo string) (tapestry *Node, err error) {
	...
	_, actualport, err := net.SplitHostPort(lis.Addr().String()) //Out.Sprintf("%v:%v", name, port)
	if err != nil {
		return nil, err
	}
	...
	if !exists {
		_, err := c.Create("/tapestry", []byte{}, 0, acl)
		if err != nil {
			return nil, err
		}
	}

	_, err = c.Create("/tapestry/"+id.String(), []byte(address), 1, acl)

	if err != nil {
		return nil, err
	}

	children, _, err := c.Children("/tapestry")
	...
	if remoteChild != "" {
		result, _, err := c.Get("/tapestry/" + remoteChild)
		if err != nil {
			return nil, err
		}
		connectTo = string(result[:])
	}
	...
	if connectTo != "" {
		...
		Out.Printf("node want to join: %v\n", node)
	...
	// child, _, _ := zkConn.Children("/tapestry")
